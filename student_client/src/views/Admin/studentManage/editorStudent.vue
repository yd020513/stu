...
export default {
  ...
  created() {
    ...
    axios.get('http://localhost:8888/student/findById/' + this.ruleForm.sid).then(function (resp) {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.post("http://localhost:8888/student/updateStudent", this.ruleForm).then(function (resp) {
            if (resp.data === true) {
              that.$message({
                showClose: true,
                message: '编辑成功',
