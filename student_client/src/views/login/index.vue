    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.get('http://localhost:8888/info/getCurrentTerm').then(function (resp) {
            sessionStorage.setItem("currentTerm", resp.data)
          })

          axios.get('http://localhost:8888/info/getForbidCourseSelection').then(function (resp) {
          ...
          if (that.ruleForm.type === 'admin' || that.ruleForm.type === 'teacher') {
            ...
            console.log(form)
            axios.post("http://localhost:8888/teacher/login", form).then(function (resp) {
              console.log("教师登陆验证信息：" + resp.data)
              check = resp.data
              if (check === true) {
                axios.get("http://localhost:8888/teacher/findById/" + that.ruleForm.id).then(function (resp) {
            ...
            axios.post("http://localhost:8888/student/login", form).then(function (resp) {
              console.log("学生登陆验证信息：" + resp.data)
              check = resp.data
              if (check === true) {
                axios.get("http://localhost:8888/student/findById/" + that.ruleForm.id).then(function (resp) {
                  console.log("登陆页正在获取用户信息" + resp.data)
